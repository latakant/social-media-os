...
class ApprovalBot:
    ...
    async def _handle_callback(
        ...
        if action == "approve":
            ...
            await query.edit_message_caption(f"Approved. Post ID: {post_id}")
            ...
            # Ask for optional suggestion — don't set event yet
            ...
            # User skipped suggestion — redraft with no extra context
            self._suggestion = None
            self._decision = "redraft"
            self._waiting_for_suggestion = False
            await query.edit_message_caption("Regenerating without additional suggestions...")
            self._event.set()
